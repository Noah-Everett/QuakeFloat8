...
(3)~Post-training quantization across the GPT-2 family (124M--774M parameters) shows QF8
matching or outperforming MXFP8 on WikiText-2 perplexity for both weight-only and W8A8 quantization.
(4)~The QF8 multiplier costs ${\sim}66$ gates vs.\ ${\sim}231$ for FP8 ($3.5\times$ cheaper).
\end{abstract}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Empirical validation.} Post-training quantization of pretrained GPT-2 models
    (124M--774M parameters) shows QF8 consistently matching or outperforming MXFP8 in WikiText-2
    perplexity, with weight-only degradation ${\leq}0.2\%$ across all sizes.
    STE-based QAT on TinyGPT-2 further confirms training viability.
\end{enumerate}
...
as $N \to \infty$. For finite~$N$, Theorem~\ref{thm:exact-msre} provides the exact (non-asymptotic)
per-cell MSRE: the ratio of exact to approximate MSRE is $0.9997$ at $N = 256$ (Table~\ref{tab:msre-verify}).
The minimax conclusion holds for all practical quantizer sizes ($N \geq 16$).
\end{remark}
...
\textbf{W8A8 PTQ.}
Quantizing activations reveals a nuance: at 124M parameters, QF8's activation penalty
($+1.73\%$) exceeds MXFP8's ($+0.22\%$), because log-uniform spacing allocates precision
uniformly across magnitudes while activations often concentrate near zero.
However, this gap narrows with scale: at 774M parameters, QF8 W8A8 degradation is only
$+0.11\%$ --- essentially identical to weight-only and comparable to MXFP8's $+0.23\%$.

\textbf{The case for relative error.}
INT8 achieves $+6.6$~dB higher SQNR than QF8 on per-tensor benchmarks
(Table~\ref{tab:sqnr-pretrained}), yet delivers comparable or worse perplexity on
weight-only PTQ (Table~\ref{tab:ptq-wonly}). At GPT-2 Medium, INT8's perplexity penalty
($+0.02\%$) exceeds QF8's ($-0.01\%$) despite INT8's large SQNR advantage.
This supports the theoretical motivation: neural network quality is better predicted by
\emph{relative} error (NMSE, which QF8 minimizes) than absolute error (MSE, which INT8 minimizes).
...
QF8 matches FP32 validation loss to within noise ($-0.02\%$), while FP8 E4M3 incurs a
small penalty ($+0.11\%$). This model is too small (124K parameters, 500 steps) to be
conclusive on its own, but combined with the PTQ results above it confirms that QF8
does not impair gradient flow during training.
...
PTQ results on GPT-2 Small through Large (124M--774M parameters) validate the format
at inference time. The remaining gap is full-scale QAT:
\begin{enumerate}[leftmargin=2em]
  \item GPT-2 Small (124M parameters, 300k steps) with STE-based QAT --- does the training
    advantage observed on TinyGPT-2 hold at standard scale?
  \item Larger models (355M+) --- do any gradient-related instabilities emerge during training?
  \item Activation-aware calibration (e.g., SmoothQuant-style channel balancing) to reduce
    QF8's W8A8 penalty at small model sizes.
\end{enumerate}
