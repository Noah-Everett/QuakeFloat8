...
accuracy at $N = 256$.
(2)~On pretrained GPT-2 weights, QF8 achieves $+6.5$~dB SQNR over block-scaled MXFP8 E4M3,
confirming the theoretical $+6.6$~dB prediction.
(3)~Weight-only post-training quantization across the GPT-2 family (124M--774M parameters)
shows QF8 matching or outperforming MXFP8 on WikiText-2 perplexity (${\leq}0.2\%$ degradation).
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Empirical validation.} Weight-only post-training quantization of pretrained GPT-2
    models (124M--774M parameters) shows QF8 matching or outperforming MXFP8 in WikiText-2
    perplexity (${\leq}0.2\%$ degradation). W8A8 quantization exposes a limitation:
    QF8's log-uniform spacing is suboptimal for activations concentrated near zero
    ($+1.7\%$ at 124M vs.\ MXFP8's $+0.2\%$), though this gap narrows with model scale.
\end{enumerate}
...
\textbf{Weight-only PTQ.}
QF8 consistently matches or outperforms MXFP8 across all three model sizes,
with degradation ${\leq}0.2\%$ everywhere.
At GPT-2 Medium (355M), the gap is most pronounced: QF8 degrades by only $-0.01\%$
vs.\ MXFP8's $+0.56\%$. These results use a single evaluation run per configuration
(no error bars); the perplexity differences at Small and Large are small enough
($0.03$--$0.07$ perplexity points) that they should be interpreted as ``comparable''
rather than as evidence of a definitive advantage for any format.
...
Weight-only PTQ results on GPT-2 Small through Large (124M--774M parameters) validate QF8
for inference-time weight quantization. Several gaps remain:
\begin{enumerate}[leftmargin=2em]
  \item \textbf{W8A8 activation handling.} QF8's log-uniform grid is structurally suboptimal for
    ...
    Characterizing the zero-code and underflow rates for activation tensors would clarify whether
    the penalty is due to clipping, underflow, or the spacing itself.
  \item \textbf{Full-scale QAT.} The TinyGPT-2 experiment (124K parameters, 500 steps) is a
    smoke test only. GPT-2 Small (124M, 300k steps) QAT with multiple seeds and error bars
    is needed before any training claim can be made.
  \item \textbf{Error bars.} All PTQ perplexity results are single-run. Perplexity differences
    at the $0.01$--$0.1\%$ level require confidence intervals (e.g., multiple evaluation strides
    or bootstrap resampling) to be meaningful.
