...
As a complementary validation, we train a TinyGPT-2 model from scratch with STE-based
quantization-aware training (QAT).

\begin{table}[h]
\centering
\caption{TinyGPT-2 QAT configuration.}
...
\begin{table}[h]
\centering
\caption{TinyGPT-2 QAT results.}
\label{tab:qat-results}
\begin{tabular}{@{}lccc@{}}
\toprule
Format & Final Train Loss & Final Val Loss & $\Delta_{\text{val}}$ vs FP32 \\
\midrule
FP32            & 2.5388 & 2.5450 & ---                          \\
...
At 124K parameters and 500 steps, this experiment is a smoke test, not a benchmark.
The validation loss differences ($-0.02\%$ for QF8, $+0.11\%$ for FP8 vs.\ FP32)
are within the step-to-step variance of the loss curve ($\sim\!0.19$ over the final 200 steps)
and should not be interpreted as evidence of QF8 superiority.
The only defensible conclusion is that STE-based QF8 quantization does not catastrophically
break training at this toy scale. Full-scale QAT validation remains future work
