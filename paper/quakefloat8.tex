...
Quantization-aware training of GPT-2 Small (162M parameters) shows QF8 matching FP32
validation loss within noise ($+0.00\%$).
...
We train GPT-2 Small (162M parameters) from scratch on WikiText-2 with STE-based
quantization-aware training (W8A8: both weights and activations quantized via
...
\begin{table}[ht]
\centering
\caption{GPT-2 Small QAT on WikiText-2 (162M parameters, 2{,}000 steps, single seed).}
\label{tab:qat}
\begin{tabular}{@{}lccc@{}}
\toprule
Format & Final Val Loss & $\Delta$ vs FP32 & ms/step (A100) \\
\midrule
FP32 (baseline) & 5.7495 & ---       & 521 \\
\textbf{QF8}    & \textbf{5.7496} & $\mathbf{+0.00\%}$ & 801 \\
FP8 E4M3        & 5.7518 & $+0.04\%$ & 1{,}111 \\
\bottomrule
\end{tabular}
\end{table}

QF8's validation loss is indistinguishable from FP32 ($\Delta = +0.0001$),
while FP8 shows a small but consistent gap ($\Delta = +0.0023$) visible
across all validation checkpoints.
The training curves for all three formats track nearly identically throughout
(Table~\ref{tab:qat}).

This is a single-seed experiment without error bars, so the $+0.04\%$ FP8 gap
may be within noise. The result confirms that STE-based QF8 quantization
introduces no measurable training degradation at 162M scale, but larger-scale
QAT with multiple seeds remains future work (Section~\ref{sec:future}).
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{QAT at scale.} The GPT-2 Small experiment (162M parameters, 2{,}000 steps)
    shows zero degradation but is single-seed and not fully converged.
