...
\subsection{Quantization-Aware Training (Preliminary)}
\label{subsec:qat}

As a preliminary check, we trained a TinyGPT-2 model (124K parameters, 500 steps)
with STE-based QAT. Both QF8 and FP8 E4M3 matched FP32 validation loss within
noise (${\leq}0.1\%$). This confirms that STE-based QF8 quantization does not
catastrophically break training, but the experiment is too small to draw any
further conclusions. Full-scale QAT remains future work (Section~\ref{sec:future}).
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Full-scale QAT.} The TinyGPT-2 experiment (124K parameters, 500 steps) is a
    smoke test only. Full-scale QAT with multiple seeds and error bars
    is needed before any training claim can be made.
