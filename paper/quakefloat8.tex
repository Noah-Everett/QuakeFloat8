...
\begin{enumerate}[leftmargin=2em]
  \item Minimax NMSE optimality theorem (Theorem~\ref{thm:minimax}).
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{``The equalization property is implicit in Bennett~[13] / Smith~[15] / Sun \& Goyal~[16].''}
    Response: Bennett's framework is entirely MSE-based; the equalization property (density-independence
    under MSRE) is structurally impossible in his framework because MSE \emph{does} depend on the
    source distribution.
    Smith's amplitude-independence result is a statement about signal level, not source distribution ---
    his distortion formula is explicitly distribution-dependent ($0.707$ for exponential, $0.798$ for Gaussian).
    Sun \& Goyal work in the MSRE framework but derive density-specific optimal quantizers;
    they do not state the equalization property or the minimax result.
    The $x^2$ cancellation in the NMSE ratio is a simple observation in retrospect,
    but it has not been stated or proved in any prior work found.
\end{enumerate}
...
For completeness, we document all corrections made relative to earlier drafts:

\begin{table}[h]
\centering
\caption{Summary of corrections from earlier drafts.}
...
Exact MSRE & $\varepsilon^2/2$ & $\varepsilon^2/12$ \\
